...
| UDP 辅助 | `49142`（0xBFF6） | UDP | 设备发现广播 + 多机状态同步（App 纯接收） |
...
- **设备发现广播**：相机向 `192.168.10.255:49142` 循环广播设备信息
- **多机状态同步**：多个手机同时连接时，通过 UDP 广播同步电量、录像状态等

**注意**：App 端 UDP 是**纯接收通道**，不通过此端口发送数据。`sendData()` 走 TCP `outStream`。

### 6.3 关于 I 帧刷新

I 帧刷新**不走 UDP:49142**。画面卡顿/花屏时的 I 帧刷新由 Native 层（`libijkplayer.so` / FFmpeg C++）通过 RTCP PLI（RFC 4585）在 RTSP 协商端口自动处理。整个 `com.record.*` 包中未找到 I-frame / IDR / keyframe 相关 Java 代码。
